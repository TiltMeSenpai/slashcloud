...
impl JsCtx {
    ...
    fn get_crypto(&self) -> web_sys::SubtleCrypto {
        self.ctx
            .crypto()
            .expect("Could not get crypto instance")
            .subtle()
    }
    ...
    pub async fn get_key(&self, cf: &worker::Env) -> Result<web_sys::CryptoKey, &str> {
        ...
        let algo = js_sys::JSON::parse(r#"{"name":"NODE-ED25519", "namedCurve": "NODE-ED25519"}"#)
            .unwrap()
            .dyn_into()
            .unwrap();
        // what the living fuck ends
        let jwk = match cf.secret("DISCORD_PUBKEY") {
            Ok(key) => js_sys::JSON::parse(&key.to_string())
                .unwrap()
                .dyn_into()
                .unwrap(),
            Err(_) => return Err("Could not find DISCORD_PUBKEY"),
        };
        let key_promise = match self
            .get_crypto()
            .import_key_with_object("jwk", &jwk, &algo, false, &usage)
        {
    ...
    pub async fn verify_request(
        ...
        match self
            ...
            .verify_with_object_and_buffer_source_and_buffer_source(
                &js_sys::JSON::parse(r#"{"name":"NODE-ED25519", "namedCurve": "NODE-ED25519"}"#)
                    .unwrap()
                    .dyn_into()
                    .unwrap(),
                key,

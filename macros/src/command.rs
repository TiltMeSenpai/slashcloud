...
impl ParsedCommandOption {
    fn to_json_quote(&self) -> TokenStream {
    ...
    fn from_json_quote(&self, name: TokenStream) -> TokenStream {
...
fn attr_is_enum(attrs: &Vec<Attribute>) -> bool {
    attrs.iter().any(|attr| match attr.path.get_ident() {
...
fn cast_quote(
    ...
    let q = match type_name {
        "String" => {
            quote!(#value_name.map(|val| val.as_str()).flatten().map(|val| val.to_string()))
        }
...
use std::iter::repeat;
pub fn derive_command_handler(ast: DeriveInput) -> TokenStream {
    let name = &ast.ident;
    ...
    let variants = match ast.data {
        ...
        _ => {
            return Error::new(Span::mixed_site(), "Can only apply CommandOption to enums")
                .to_compile_error()
                .into()
        }
    };
    if is_enum {
        let parsed_enum = variants.iter().map(|variant| ParsedCommandEnum {
                ...
                .map(|field| {
                    if let Type::Path(path) = &field.ty {
                        ...
                        let (code, ty, options) = if is_enum {
                            (
                                ParsedCommandType::String,
                                ty.to_owned(),
                                ParsedCommandExtras::Choices(ty.to_owned()),
                            )
                        } else {
                        ...
                        ParsedCommandOption {
                            ...
                            name: field
                                .ident
                                .as_ref()
                                .expect("Only named fields (struct style enums) supported")
                                .to_owned(),
                            description: desc,
                            required: required,
                            options,
        ...
        let to_value_quotes = parsed_option
            .to_owned()
            .map(|option| option.to_json_quote());
        let from_json_quotes = parsed_option
            .to_owned()
            .map(|option| option.from_json_quote(quote!(options)));
        quote! {
            impl CommandOption for #name {
                fn from_value(options: &serde_json::Value) -> Option<Self>{
                    let arg_iter: Vec<(&str, &serde_json::Value)> = options["options"]
                        .as_array().map( | array |
                            array.iter().map(
                                | option | (option["name"].as_str().unwrap(), &option["value"])
                            ).collect()
                        ).unwrap_or_default();
